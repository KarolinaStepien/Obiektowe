...
public class AdminUnitQuery {
    ...
    protected Predicate<AdminUnit> p = a -> true;
    protected Comparator<AdminUnit> cmp;
    ...
    AdminUnitList execute() {
        return new AdminUnitList(src.units.stream().filter(p).skip(offset).limit(limit).sorted(cmp));
    }
}
